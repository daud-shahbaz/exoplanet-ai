...
def main():
    ...
    print(metrics)
    save_outputs(OUTDIR, "gb", metrics)
    ...
    plt.tight_layout()
    plt.savefig(f"{OUTDIR}/gradeint_boosting_confusion_matrix.png")
    plt.close()
    ...
    plt.title("Performance Metrics - Gradient Boosting")
    plt.ylabel("Score")
    plt.ylim(0,1)
    plt.xticks(rotation=30)
    plt.tight_layout()
    plt.savefig(f"{OUTDIR}/gb_scores.png")
    plt.close()
